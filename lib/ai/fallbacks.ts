...
export function fallbackProposedSolution(input: ProposalInput) {
  return {
    overview:
      'We propose a three-layer architecture: (1) a capability layer exposing AI-powered services via clean APIs, (2) a unified context and data layer serving both AI and human operators, and (3) a governance layer providing audit trails, decision logging, and compliance reporting. All layers deploy into your existing cloud footprint.',
    components: [
      { name: 'Intelligence Layer', description: 'LLM-driven orchestration, retrieval, and tool execution — the decision core of the platform.' },
      ...
      { name: 'Operator Console', description: 'Modern web UI giving operators full visibility into AI decisions and one-click takeover when needed.' },
      { name: 'Compliance & Audit', description: 'Immutable logging, bias monitoring, and regulator-ready report export.' },
      { name: 'Observability & SRE', description: 'Datadog-grade monitoring, alerting, and runbooks for 24×7 operations.' },
    ],
    integrations: [
      'Client source-of-truth systems — for context and state',
      'Identity & auth systems — OTP, SSO, step-up authentication',
      'Analytics / BI — event export for downstream reporting',
      'Communications systems — telephony, email, chat channels',
      'Security & audit — SIEM / log sinks',
    ],
    techStack: input.preferredTechnologies?.length
      ? input.preferredTechnologies
      : ['TypeScript', 'Next.js', 'Python', 'FastAPI', 'PostgreSQL', 'Redis', 'AWS', 'Terraform', 'Datadog'],
    innovations: [
...
export function fallbackDeliverables(phaseNames: string[]) {
  return {
    items: [
      { phase: phaseNames[0] ?? 'Phase 1', name: 'Solution Blueprint', description: 'Complete architecture, integrations, and compliance mapping.', acceptance: 'Signed off by client architecture review board.' },
      { phase: phaseNames[0] ?? 'Phase 1', name: 'Project Plan & Risk Register', description: 'Detailed plan with resource plan, risks, and mitigation.', acceptance: 'Approved by steering committee.' },
      { phase: phaseNames[1] ?? 'Phase 2', name: 'Core Services Live', description: 'Context and integration services deployed to staging.', acceptance: 'All integration SLAs met; load-tested to 3× peak.' },
      { phase: phaseNames[1] ?? 'Phase 2', name: 'Identity & Auth', description: 'SSO, step-up auth, and audit logging in place.', acceptance: 'Successful end-to-end auth flow with audit trail.' },
      { phase: phaseNames[2] ?? 'Phase 3', name: 'Primary Use Cases', description: 'Top use cases delivered and shadow-tested in production.', acceptance: 'Shadow metrics meet acceptance thresholds.' },
      { phase: phaseNames[2] ?? 'Phase 3', name: 'Operator Console', description: 'Full operator UI with takeover and escalation.', acceptance: 'Operators trained and signed off.' },
      { phase: phaseNames[3] ?? 'Phase 4', name: 'Security & Compliance Pack', description: 'Pen-test, bias audit, compliance dossier.', acceptance: 'No criticals; regulator walkthrough complete.' },
      { phase: phaseNames[4] ?? 'Phase 5', name: 'Production Rollout', description: 'Full production deployment with runbooks and on-call.', acceptance: '100% eligible traffic live; 30-day SLA met.' },
    ],
    milestones: [
      { name: 'Design Sign-off', target: 'Week 4' },
      { name: 'Foundation Acceptance', target: 'Week 9' },
      { name: 'Alpha Release', target: 'Week 14' },
...
export function fallbackRiskMitigation() {
  return {
    introduction:
      'We have run a structured risk assessment across delivery, technology, regulatory, adoption, and vendor axes. The following are the top risks we actively manage with named mitigations.',
    risks: [
      { id: 'R-01', title: 'Integration delays with legacy systems', description: 'Dependencies on client IT may create unplanned wait states.', probability: 'Medium', impact: 'High', mitigation: 'Early integration spike in Week 1; named client counterpart; contingency week in Phase 2.' },
      { id: 'R-02', title: 'Data quality in source systems', description: 'Inconsistent or incomplete records can degrade downstream accuracy.', probability: 'High', impact: 'Medium', mitigation: 'Data profiling in Week 2; automated reconciliation layer; SLAs on source quality.' },
      { id: 'R-03', title: 'Regulatory interpretation change', description: 'Guidance in this domain continues to evolve.', probability: 'Low', impact: 'High', mitigation: 'Quarterly regulatory review; modular architecture; dedicated compliance liaison.' },
      { id: 'R-04', title: 'Operator adoption & change resistance', description: 'End users may resist new tooling or workflows.', probability: 'Medium', impact: 'Medium', mitigation: 'Co-design from Week 2; champion program; phased rollout with feedback loops.' },
      { id: 'R-05', title: 'Latency & scale regressions', description: 'Performance at production load may differ from staging.', probability: 'Medium', impact: 'Medium', mitigation: 'Weekly 3× peak load tests; autoscaling; per-use-case SLOs.' },
      { id: 'R-06', title: 'Third-party vendor incident', description: 'Upstream outage could cascade to the platform.', probability: 'Low', impact: 'High', mitigation: 'Dual-vendor fallback chain; graceful degradation path; clear runbooks.' },
      { id: 'R-07', title: 'Scope drift', description: 'Mid-project scope change can jeopardize timeline and cost.', probability: 'Medium', impact: 'Medium', mitigation: 'Formal CR process; steering committee approval; impact-assessment template.' },
    ],
  };
}
